...
parser.add_argument('--max_length', default=50, type=int, help='Maximum sentence length.')
...
def main():
    ...
    if not os.path.exists('ckpt'):
        os.mkdir('ckpt')
    if len(args.ckpt) == 0:
        args.ckpt = '{}_{}'.format(args.dataset, 'bere_' + args.bag_strategy)
    ...
    tag2id = {tag: ix for ix, tag in enumerate(tag_dict.keys())}

    # Define the sentence encoder
    sentence_encoder = opennre.encoder.BEREEncoder(
        token2id=word2id,
        tag2id=tag2id,
    ...
    if args.bag_strategy == 'att':  # use attention strategy to aggregate sentences in bag
        model = opennre.model.BagBEREAttention(sentence_encoder, len(rel2id), rel2id)
    else:  # use average instead
        model = opennre.model.BagBEREAverage(sentence_encoder, len(rel2id), rel2id)
