...
def main():
    ...
    if not os.path.exists('ckpt'):
        os.mkdir('ckpt')
    if len(args.ckpt) == 0:
        args.ckpt = '{}_{}'.format(args.dataset, 'gruatt_' + args.bag_strategy)
    ...
    sentence_encoder = opennre.encoder.GRUAttEncoder(
        token2id=word2id,
    ...
    if args.bag_strategy == 'att':  # use attention strategy to aggregate sentences in bag
        model = opennre.model.BagGRUAttention(sentence_encoder, len(rel2id), rel2id)
    else:  # use average instead
        model = opennre.model.BagGRUAverage(sentence_encoder, len(rel2id), rel2id)
