...
class WordTokenizer(object):
    ...
    def __init__(self, vocab = None, unk_token="[UNK]", tag_vocab = None):
        self.vocab = load_vocab(vocab)
        self.inv_vocab = {v: k for k, v in self.vocab.items()}
        self.unk_token = unk_token
        if tag_vocab:
            self.tag_vocab = load_vocab(tag_vocab)
    ...
    def convert_tags_to_ids(self, tags, max_seq_length = None, blank_id = 0, unk_id = 1, uncased = False):
        return convert_by_vocab(self.tag_vocab, tags, max_seq_length, blank_id, unk_id, uncased=uncased)

    def convert_ids_to_tags(self, ids):
        return convert_by_vocab(self.inv_tag_vocab, ids, uncased=False)
