...
class BagGRUAttention(BagRE):
    ...
    def forward(self, label, scope, token, mask=None, train=True, bag_size=0):
        """
        Args:
            label: (B), label of the bag
            scope: (B), scope for each bag
...
class BagBEREAttention(BagRE):
    ...
    def __init__(self, sentence_encoder, num_class, rel2id):
        ...
        self.scale = self.sentence_encoder.hidden_size ** -0.5
        self.att_w = nn.Parameter(torch.FloatTensor(self.sentence_encoder.hidden_size))
    ...
    def reset_parameters(self):
        nn.init.normal_(self.att_w.data, mean=0, std=0.01)
    ...
    def infer(self, bag):
        ...
        for item in bag:
            ...
            masks.append(mask)
        tokens = torch.cat(tokens, 0).unsqueeze(0) # (n, L)
        tags = torch.cat(tags, 0).unsqueeze(0)
        masks = torch.cat(masks, 0).unsqueeze(0)
        scope = torch.tensor([[0, len(bag)]]).long() # (1, 2)
    ...
    def forward(self, label, scope, token, tag, mask=None, train=True, bag_size=0):
        """
        Args:
            label: (B), label of the bag
            scope: (B), scope for each bag
        ...
        # Attention
        if train:
            if bag_size == 0:  # TODO: fix attention when bag_size == 0 too
                bag_rep = []
                query = torch.zeros((rep.size(0))).long()
                if torch.cuda.is_available():
                    query = query.cuda()
                for i in range(len(scope)):
                    query[scope[i][0]:scope[i][1]] = label[i]
                att_mat = self.fc[0].weight[query] # (nsum, H)
                att_score = (rep * att_mat).sum(-1) # (nsum)
                ...
                batch_size = label.size(0)
                rep = rep.view(batch_size, bag_size, -1) # (B, bag, H)
                att_score = (self.att_w * rep).sum(-1) # (B, bag)
                att_score = self.scale * att_score
                softmax_att_score = self.softmax(att_score) # (B, bag)
                bag_rep = (softmax_att_score.unsqueeze(-1) * rep).sum(1) # (B, bag, 1) * (B, bag, H) -> (B, bag, H) -> (B, H)
            bag_rep = self.drop(bag_rep)
            bag_logits = self.fc(bag_rep) # (B, N)
            ...
            if bag_size == 0:  # TODO: fix attention when bag_size == 0 too
                bag_logits = []
                att_score = torch.matmul(rep, self.fc[0].weight.transpose(0, 1)) # (nsum, H) * (H, N) -> (nsum, N)
                for i in range(len(scope)):
                    bag_mat = rep[scope[i][0]:scope[i][1]] # (n, H)
                    softmax_att_score = self.softmax(att_score[scope[i][0]:scope[i][1]].transpose(0, 1)) # (N, (softmax)n)
                    rep_for_each_rel = torch.matmul(softmax_att_score, bag_mat) # (N, n) * (n, H) -> (N, H)
