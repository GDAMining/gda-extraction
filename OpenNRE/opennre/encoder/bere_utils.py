...
def non_padding_mask(token, pad_id):
    non_pad_mask = token.ne(pad_id).type(torch.float)
...
def padding_mask(token, pad_id):
    len_q = token.size(1)
    pad_mask = token.eq(pad_id).unsqueeze(1).expand(-1, len_q, -1)  # shape [B, L_q, L_k]
    return pad_mask
